...
public class Engine {
    ...
    public static void main(String[] args) throws Exception{
    	...
    	while (true) {
            thread(new Engine_consumer(), false);
            thread(new Engine_consumer(), false);
            thread(new Engine_consumer(), false);
            thread(new Engine_consumer(), false);
            thread(new Engine_consumer(), false);
    ...
    public static class Engine_consumer implements Runnable, ExceptionListener {
        public void run() {
        try {
            // URL of the JMS server
            //String url = ActiveMQConnection.DEFAULT_BROKER_URL;
            String url = "tcp://localhost:61616";
            String queue = "ENGINE_QUEUE";
            ...
            if (msg instanceof TextMessage) {
				...
				case 5: // Emergency Button
					...
					switch (n){
					case 1:
						String Message_fire_emergency = ("Alert for Fire from button Emergency, Notifying FireFighters. Event received =" + text);
						SendQueueMsg("tcp://localhost:61616", "FIRE_QUEUE", Message_fire_emergency);
						System.out.println(Message_fire_emergency);
						break;
					case 2:
						String Message_steal_emergency = ("Alert for Vehicle Robbery from button Emergency, Notifying Police. Event received =" + text);
						SendQueueMsg("tcp://localhost:61616", "STEAL_QUEUE", Message_steal_emergency);
						System.out.println(Message_steal_emergency);
						break;
					default:
						String Message_acc_emergency = ("Alert for Car Accident from button Emergency. Event received = " + text);
				...
				default: // Info about Car State
					String  Message_info = ("Info about car state. Event received = "+ text);
